...
def make_wrap_band(name, center, axis_dir, radius, minor_r=0.006):
    """A thin buckle-band torus wrapped around the pouch, oriented along axis_dir."""
    bpy.ops.mesh.primitive_torus_add(
        major_radius=radius, minor_radius=minor_r,
        location=(0, 0, 0), major_segments=16, minor_segments=6,
    )
...
for t in (0.30, 0.68):
    ...
    band_r = radius_at(t) + 0.006
    band_objs.append(make_wrap_band(f"QuiverBand{t}", band_center, axis_dir, band_r))

# small connector strap tying the lower band back to the main bandolier strap
connector_obj = make_strap(
    attach_point + mathutils.Vector((0.0, 0.0, 0.0)),
    pouch_base + axis_dir * (pouch_length * 0.30),
    width=0.03, thickness=0.012, name="QuiverConnector",
)
...
connector_obj.data.materials.append(leather_mat)
for band in band_objs:
    band.data.materials.append(leather_mat)
for b in bolt_objs:
...
all_objs = [body_obj, strap_obj, pouch_obj, connector_obj] + band_objs + bolt_objs
allx, ally, allz = [], [], []
for o in all_objs:
